# Lesson 2: Protected Routes (Long-form Enhanced)

> Protected routes are mostly a **UX concern** in the frontend. Real security is enforced by backend auth/authz. This long-form lesson focuses on choosing the right layer (middleware/layout/client) for the best user experience with the least risk.

## Table of Contents

- Protected routes vs real security
- Client-side wrappers (UX-only)
- Middleware-based redirects (cookie-based gating)
- Layout-level gating patterns
...
### 1) “Return to” redirects (better UX)

If a user hits `/dashboard` while logged out, store the destination:
- redirect to `/login?returnTo=/dashboard`
- after login, redirect back to `returnTo`

### 2) Avoid leaking protected data during SSR

Even if you redirect quickly, ensure protected data is never fetched/shipped to the browser when unauthenticated.
Backend endpoints must enforce auth, and server-side fetching should respect auth state.
